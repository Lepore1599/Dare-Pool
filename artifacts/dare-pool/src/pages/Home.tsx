        ...
        {(["active", "ended"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-all ${
              filter === f ? "bg-primary text-white glow-primary-sm" : "bg-secondary text-muted-foreground hover:text-foreground"
        ...
        <motion.div key={filter} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-muted-foreground">
              ...
              {filter === "active" && (
                <p className="text-sm mt-1">Be the first — <Link href="/create" className="text-primary hover:underline">create one</Link></p>
              )}
            </div>
          ) : (
            displayed.map((dare, i) => <DareCard key={dare.id} dare={dare} index={i} />)
          )}
        </motion.div>
      </AnimatePresence>
    </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <Link href={`/dare/${dare.id}`}>
        <div className="group bg-card border border-card-border rounded-2xl p-4 hover:border-primary/40 transition-all hover:shadow-lg cursor-pointer"
